...
const HistorialScreen = React.memo(function HistorialScreen() {
  ...
  const onDownload = async () => {
    try {
      ...
      // Cargar información real del perfil del usuario
      let patientInfo = {
      ...
      // Si el usuario está autenticado, cargar su perfil
      if (user?.mode === "user") {
        try {
          const userProfile = await loadProfileFromSupabase();
          if (userProfile) {
            patientInfo = {
              name: userProfile.name,
              age: userProfile.age,
              sex: userProfile.gender || "No especificado"
            };
          }
        } catch (error) {
          console.error('[Historial] Error al cargar perfil:', error);
      ...
      const csv = useSupabase 
        ? supabaseHistoryToCSV(dataToUse as SupabaseHistoryEntry[], patientInfo)
        : historyToCSV(dataToUse as HistoryEntry[], patientInfo);
