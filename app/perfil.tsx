...
export default function Perfil() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (next.hasSession && user) {
        ...
        // Usar el perfil cargado directamente
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Si hay una nueva imagen seleccionada, subirla a Supabase Storage
      if (selectedImage) {
        console.log('[Perfil] Nueva imagen detectada:', selectedImage);
        console.log('[Perfil] Subiendo nueva imagen...');
        const uploadedUrl = await uploadImageToSupabase(selectedImage);
        if (uploadedUrl) {
          avatarUrl = uploadedUrl;
          console.log('[Perfil] ✅ Imagen subida exitosamente:', uploadedUrl);
        } else {
          console.warn('[Perfil] ⚠️ No se pudo subir la imagen, usando URL anterior:', profile?.avatar_url);
        }
      } else {
        console.log('[Perfil] No hay nueva imagen, usando URL existente:', profile?.avatar_url);
      }

      // Crear perfil para Supabase (incluyendo gender)
      ...
      if (result) {
        // Actualizar el perfil local (result ya incluye el gender)
        setProfile(result);
  ...
  const uploadImageToSupabase = async (imageUri: string): Promise<string | null> => {
    try {
      console.log('[Perfil] Iniciando subida de imagen...', imageUri);
      
      // Obtener el usuario actual
      const { data: user, error: userError } = await supabase.auth.getUser();
      if (userError || !user.user) {
        console.error('[Perfil] Error al obtener usuario:', userError);
        throw new Error('No hay usuario autenticado');
      }

      console.log('[Perfil] Usuario autenticado:', user.user.id);

      // Crear nombre único para el archivo
      const timestamp = Date.now();
      const fileName = `avatar_${user.user.id}_${timestamp}.jpg`;
      const filePath = `avatars/${fileName}`;

      console.log('[Perfil] Preparando archivo:', fileName);
      ...
      console.log('[Perfil] Blob creado, tamaño:', blob.size);

      // Intentar subir a Supabase Storage con diferentes buckets
      const buckets = ['profile-images', 'public', 'avatars'];
      
      for (const bucket of buckets) {
        try {
          console.log(`[Perfil] Intentando con bucket: ${bucket}`);
          ...
          if (error) {
            console.log(`[Perfil] Error con bucket ${bucket}:`, error.message);
          ...
          console.log(`[Perfil] ✅ Imagen subida exitosamente con bucket ${bucket}:`, urlData.publicUrl);
          return urlData.publicUrl;
        } catch (bucketError) {
          console.log(`[Perfil] Error con bucket ${bucket}:`, bucketError);
          continue; // Intentar siguiente bucket
        }
      }

      // Si todos los buckets fallan, usar la URI local como fallback
      console.warn('[Perfil] ⚠️ No se pudo subir a ningún bucket, usando URI local como fallback');
      return imageUri;

    } catch (error) {
      console.error('[Perfil] Error completo al subir imagen:', error);
      // Como último recurso, devolver la URI local
      console.warn('[Perfil] ⚠️ Usando URI local como fallback:', imageUri);
      return imageUri;
    }
  };
