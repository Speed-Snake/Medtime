...
export async function scheduleMedicationNotificationWithAlarm(medication: MedItem, scheduledTime: string): Promise<string | null> {
  try {
    ...
    // NO modificar la hora programada - respetar la hora exacta
    // Solo asegurar que sea al menos 1 segundo en el futuro
    ...
    // Generar ID único basado en medicamento y hora programada (sin timestamp para evitar duplicados)
    const timeKey = scheduledTime.replace(/[:.]/g, '_');
    const notificationId = `${medication.id}_${timeKey}`;
    
    console.log(`[NotificationService] Fecha programada: ${triggerDate.toISOString()}`);
    console.log(`[NotificationService] Diferencia en segundos: ${(triggerDate.getTime() - now.getTime()) / 1000}`);
    console.log(`[NotificationService] Hora programada: ${triggerDate.toLocaleTimeString()}`);
    console.log(`[NotificationService] Hora actual: ${now.toLocaleTimeString()}`);
    console.log(`[NotificationService] Hora solicitada: ${scheduledTime}`);
    console.log(`[NotificationService] Horas extraídas: ${hours}:${minutes}`);
    
    // Cancelar notificación existente con el mismo ID para evitar duplicados
    try {
      await Notifications.cancelScheduledNotificationAsync(notificationId);
      console.log(`[NotificationService] Notificación existente cancelada: ${notificationId}`);
    } catch (error) {
      // No es un error si no existe la notificación
      console.log(`[NotificationService] No había notificación existente para cancelar`);
    }
...
export async function scheduleReminderNotifications(medication: MedItem, scheduledTime: string): Promise<string[]> {
  try {
    ...
    // NO programar notificación principal aquí para evitar duplicados
    // La notificación principal se programa por separado
    
    // TEMPORALMENTE DESHABILITADO: Programar recordatorios adicionales si está configurado
    if (false && alarmSettings.reminderInterval > 0) {
      const now = new Date();
      let hours, minutes;
...
async function handleTakeMedication(data: any): Promise<void> {
  try {
    ...
    console.log('[NotificationService] 🔍 DEBUG - Data recibida:', JSON.stringify(data, null, 2));
    
    // Importar las funciones necesarias
    const { addToHistory } = await import('../storage/history');
    const { findMedicineById } = await import('../storage/localMedicines');
    const { removeSpecificTimeFromMedicine } = await import('../storage/localMedicines');
    
    // Obtener el medicamento completo para acceder a selectedDates
    console.log('[NotificationService] 🔍 DEBUG - Buscando medicamento con ID:', data?.medicationId);
    const medication = await findMedicineById(data?.medicationId);
    
    if (!medication) {
      console.error('[NotificationService] ❌ Medicamento no encontrado:', data?.medicationId);
      return;
    }
    
    console.log('[NotificationService] 🔍 DEBUG - Medicamento encontrado:', JSON.stringify(medication, null, 2));
    console.log('[NotificationService] 🔍 DEBUG - selectedDates:', medication.selectedDates);
    
    // Eliminar la hora específica del medicamento
    const timeToRemove = data.scheduledTime;
    if (timeToRemove) {
      await removeSpecificTimeFromMedicine(data?.medicationId, timeToRemove, medication.owner);
      console.log('[NotificationService] Hora eliminada del medicamento:', timeToRemove);
    }
    
    // Agregar al historial con selectedDates si existen
    const historyEntry = {
    ...
    console.log('[NotificationService] 🔍 DEBUG - Entrada de historial a guardar:', JSON.stringify(historyEntry, null, 2));
...
async function handleCancelMedication(data: any): Promise<void> {
  try {
    ...
    // Agregar al historial como cancelado con selectedDates si existen
    const historyEntry = {
      id: Date.now().toString(),
      name: data.medicationName || 'Medicamento',
      dose: data.dose || 'N/A',
